...
def reload_dataset(num_frames):
    ...
    test_dataset = UVGDataset(num_frame=num_frames)
...
def train_one_epoch(model, criterion, train_dataloader, optimizer, epoch, clip_max_norm, args):
    ...
    #     rd_weight = [0.2, 0.5, 1.2]
    #     lambda_I = [435, 845, 1625, 3140, 6060]
    #     lambda_P = [1024]
        rd_weight = [0, 1.0]
        lambda_I = [435, 845, 1625, 6060]
        lambda_P = [2048]
    elif args.num_frames == 3:
        ...
        rd_weight = [0.2, 0.5, 1.2, 0.5, 0.9, 0.5, 1.1, 0.5]
        lambda_I = [435, 845, 1625, 3140, 6060]
    ...
    for i, d in enumerate(train_dataloader):
        ...
        for frame_idx, frame in enumerate(d):
            ...
            if frame_idx == 0:
                ...
                out_criterion = criterion(out, frame, lambda_P[-1], rd_weight[frame_idx])
            rd_loss_per_seq += out_criterion["loss"]
    ...
    with torch.no_grad():
        ...
        for i, folder in enumerate(test_dataloader):
            ...
            for frame_idx, frame in enumerate(frames):
                ...
                out_criterion = criterion(out, frame, 2048)
